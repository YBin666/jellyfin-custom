...
if touch /sdcard/.test_has_read_write_media &>/dev/null; then
    mkdir -p /sdcard/{Download,DCIM,Movies,Music,Pictures,MediaLib}
    MEDIA_DIR_CONFIG=" -v /sdcard/Download -v /sdcard/DCIM -v /sdcard/Movies -v /sdcard/Music -v /sdcard/Pictures -v /sdcard/MediaLib $MEDIA_DIR_CONFIG "
...
if [ -n "$1" ]; then
  unset cmd
  cmd="$*"
  udocker_run --entrypoint "bash -c" -p "$PORT:8096" -e DOTNET_GCHeapHardLimit="1C0000000" -e LD_PRELOAD= -v "$DATA_DIR/config:/config" -v "$DATA_DIR/cache:/cache" $MEDIA_DIR_CONFIG "$CONTAINER_NAME" "export LD_PRELOAD=/usr/lib/jellyfin/libjemalloc.so.2; $cmd"
else
  udocker_run --entrypoint "bash -c" -p "$PORT:8096" -e _PORT="$PORT" -e DOTNET_GCHeapHardLimit="1C0000000" -e LD_PRELOAD= -v "$DATA_DIR/config:/config" -v "$DATA_DIR/cache:/cache" $MEDIA_DIR_CONFIG "$CONTAINER_NAME" ' \
